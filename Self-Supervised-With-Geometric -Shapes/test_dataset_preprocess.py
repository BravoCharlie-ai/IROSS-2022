...
def get_pointcloud(dets,seq_id,frame_id,root_dir,points_per_object):
    ...
    for box in boxes:
        center=box[0:3]
        e=box[3:6]
        e=np.array([e[0],e[2],e[1]])
        #e=np.array([e[2],e[0],e[1]])
        ...
        choice = np.random.choice(len(bbox_point), points_per_object, replace=True)

        bbox_points_scaled.append(bbox_point[:,0:3][choice, :])
