...
def main():
	...
	if os.path.exists(os.path.join(save_weights, 'prev_state.pth')):
		prev_states = torch.load(os.path.join(save_folder, 'prev_state.pth'))
	...
	train_dataset=TrainDataset(config.root_dir+'sequences/',label_dir=config.train_label_dir, sample_max_len=2,points_per_object=512)
	...
	for i, (input, det_info, det_id, det_cls,det_split) in enumerate(train_loader):
		...
		if current_step%(train_data_length*1)==0:
			...
			torch.save(net.state_dict(),
					   os.path.join(
						   save_weights,
						   'ss_JRDB_IROS_' +'epoch_'+ repr(actual_epoch)+ '_Loss_ %.4f' % (loss.item())+'.pth'))
