...
import torch
from eval_dataset_preprocess import get_pointcloud,create_frame_detections,get_frame_det_info,get_train_names,get_test_names,get_val_names,get_val_names_check

class TestSequenceDataset(object):
    def __init__(self, root_dir,label_dir,points_per_object, fix_iou=0.2, fix_count=2, gt_ratio=0, sample_max_len=2):
    ...
    def _generate_meta_seq(self):
        ...
        for seq_id in self.val_seqs:
            ...
            metas.append(TestSequence(name, self.root_dir,self.points_per_object,det_seq, self.get_pointcloud,interval=2))
        return metas    


class TestSequence(Dataset):
    def __init__(self,name, root_dir, points_per_object, det_frames, get_pointcloud_partial, interval=2):
    ...
    def _generate_lidar(self, idx):
        ...
        for frame in frames:
            ...
            point_cloud=get_pointcloud(frame['detection'],seq_id,frame_id,self.root_dir,self.points_per_object)
            ...
            det_info['boxes'].append(torch.Tensor(point_cloud['boxes']))
